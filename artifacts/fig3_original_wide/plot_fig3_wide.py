...
FAULT_COLORS={'zero_imu':'#B9A0D5','no_update':'#7FB99C'}
metadata_by_method={m:json.loads((ROOT/m/'metadata.json').read_text()) for m in ('hybrid','mlp_only')}
SCHEDULE=metadata_by_method['hybrid']['dropout_sticking_schedule']
assert SCHEDULE==metadata_by_method['mlp_only']['dropout_sticking_schedule']
for method,meta in metadata_by_method.items():
 recorded=np.load(ROOT/method/'timeseries.npz',allow_pickle=False)
 expected=np.zeros(len(recorded['time_s']),dtype=int)
 for event in SCHEDULE:
  a,b=event['start_step'],event['end_step']
  expected[a:b]=meta['fault_code_mapping'][event['subtype']]
  values=recorded['policy_imu_observation'][a:b]
  assert np.allclose(values,0 if event['subtype']=='zero_imu' else values[0])
 assert np.array_equal(recorded['fault_code'],expected)
 assert np.array_equal(recorded['fault_active'],expected!=0)
def shade_schedule(ax,alpha,zorder):
 for event in SCHEDULE:
  ax.axvspan(event['start_s'],event['end_s'],color=FAULT_COLORS[event['subtype']],alpha=alpha,lw=0,zorder=zorder)
...
fig.legend(handles=[
 ...
 Patch(facecolor=FAULT_COLORS['zero_imu'],label='Signal-Loss Fault'),
 Patch(facecolor=FAULT_COLORS['no_update'],label='Frozen-Signal Fault')],loc='upper center',
 bbox_to_anchor=(.55,.998),ncol=5,frameon=False,fontsize=15.5,
 handlelength=2,columnspacing=1.2,borderaxespad=0.15)
...
for i,(method,title,color) in enumerate(zip(('hybrid','mlp_only'),('(a) LocoDyad','(b) MLP-E2E (Aux)'),COLORS)):
 ...
 fault_axis=fig.add_axes([left,2.78/HEIGHT,.405,.10/HEIGHT])
 fault_axis.set_xlim(0,7);fault_axis.set_ylim(0,1);fault_axis.set_axis_off()
 for event in SCHEDULE:
  fault_axis.broken_barh([(event['start_s'],event['end_s']-event['start_s'])],(0,1),facecolors=FAULT_COLORS[event['subtype']],edgecolors='white',linewidth=.4)
 fault_axis.text(1.95,.5,'2.00 s',ha='right',va='center',fontsize=12,weight='semibold')
 fault_axis.text(4.05,.5,'4.00 s',ha='left',va='center',fontsize=12,weight='semibold')
 if i==0:
  fault_axis.text(-.09,.5,'IMU fault',transform=fault_axis.transAxes,ha='right',va='center',fontsize=13,weight='semibold')
...
manifest['fault_schedule']=SCHEDULE
manifest['fault_channels']=['base_angular_velocity_x','base_angular_velocity_y','base_angular_velocity_z','projected_gravity_x','projected_gravity_y','projected_gravity_z']
(OUT/'manifest.json').write_text(json.dumps(manifest,indent=2)+'\n')
print('Saved original 1085.2 x 441.02 pt page size with 6 enlarged snapshots.')

import csv
with (OUT/'fig3_fault_schedule.csv').open('w',newline='') as f:
 writer=csv.writer(f)
 writer.writerow(['start_s_inclusive','end_s_exclusive','duration_s','fault_type'])
 for event in SCHEDULE:
  writer.writerow([f"{event['start_s']:.2f}",f"{event['end_s']:.2f}",f"{event['end_s']-event['start_s']:.2f}",event['subtype']])
