...
for i,(method,title,color) in enumerate(zip(('hybrid','mlp_only'),('(a) LocoDyad','(b) MLP-E2E (Aux)'),COLORS)):
 ...
 fig.text(.04,(4.90-offset)/HEIGHT,title,color=color,size=9.5,weight='bold',va='bottom')
 for j,(ts,stage) in enumerate(zip(TIMES,STAGES)):
  ...
  ax=fig.add_axes([.04+.32*j,(4.27-offset)/HEIGHT,.285,.56/HEIGHT])
  ...
  fig.text(.04+.32*j+.285/2,(4.14-offset)/HEIGHT,f'{stage}: {ts:g} s',ha='center',va='baseline',size=7.5,weight='semibold')
 velocity=fig.add_axes([.25,(3.71-offset)/HEIGHT,.725,.38/HEIGHT])
 yaw=fig.add_axes([.25,(3.26-offset)/HEIGHT,.725,.38/HEIGHT])
 contact=fig.add_axes([.25,(2.83-offset)/HEIGHT,.725,.36/HEIGHT])
 ...
 contact.yaxis.set_label_coords(-.11,.5)
 if i == 1:
...
print('Saved 3.5 x 5.2 inch figure with original 700-sample traces and 6 snapshots.')
