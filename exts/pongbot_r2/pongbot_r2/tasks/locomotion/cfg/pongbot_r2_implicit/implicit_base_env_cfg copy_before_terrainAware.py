...
class RewardsCfg:
    ...
    test_gait_reward = RewTerm(
        ...
        weight=0.0, #0.25,
        params={
            "tracking_contacts_shaped_force": -3.0, # -2.0,
            "tracking_contacts_shaped_vel": -3.0,   # -2.0,
    ...
    pen_joint_torque = RewTerm(func=mdp.joint_torques_l2, weight=-2.5e-6) #2.5e-6) 
    ...
    # pen_joint_vel_l2 = RewTerm(func=mdp.joint_vel_l2, weight= -5e-4)
    pen_joint_accel = RewTerm(func=mdp.joint_acc_l2, weight=-7.5e-8) #-1.0e-7)
    # pen_action_rate = RewTerm(func=mdp.action_rate_l2_clamped, weight=-0.01, params={"max_value": 400.0})
    pen_action_rate = RewTerm(func=mdp.action_rate_l2_clamped, weight=-0.0025, params={"max_value": 400.0})
    # pen_action_rate = RewTerm(func=mdp.action_rate_l2_clamped, weight=-1.5e-5, params={"max_value": 400.0})
    ...
    pen_action_smoothness = RewTerm(func=mdp.ActionSmoothnessPenalty, weight=-0.0025) #-1.5e-5) #-0.0025)
    pen_joint_powers = RewTerm(func=mdp.joint_powers_l1, weight=-5.0e-5)
    pen_joint_powers_var = RewTerm(func=mdp.joint_powers_var, weight=-5.5e-6)
