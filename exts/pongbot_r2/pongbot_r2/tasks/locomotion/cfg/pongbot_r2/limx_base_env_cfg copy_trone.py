...
class EventsCfg:
    ...
    robot_joint_stiffness_and_damping = EventTerm(
        ...
        params={
            ...
            "damping_distribution_params": (2.5, 2.5),
            "operation": "abs",
...
class RewardsCfg:
    ...
    # ===== Fixed Auxiliary =====
    # ===== 
    pen_lin_vel_z = RewTerm(func=mdp.lin_vel_z_l2, weight=-4e-4)
    ...
    pen_undesired_contacts = RewTerm(
        ...
        weight= -0.5,
        params={
            "sensor_cfg": SceneEntityCfg("contact_forces", body_names=[".*THIGH", "BODY"]),
            "threshold": 10.0,
        },
    )
    pen_joint_pos_limits = RewTerm(func=mdp.joint_pos_limits, weight=-0.2)
    pen_joint_torque = RewTerm(func=mdp.joint_torques_l2, weight=-5e-6)
    pen_joint_vel_l2 = RewTerm(func=mdp.joint_vel_l2, weight=-1e-3)
    pen_joint_accel = RewTerm(func=mdp.joint_acc_l2, weight=-1e-7)
    pen_action_rate = RewTerm(func=mdp.action_rate_l2, weight=-2e-3)
    pen_action_smoothness = RewTerm(func=mdp.ActionSmoothnessPenalty, weight=-2e-3)
    pen_joint_powers = RewTerm(func=mdp.joint_powers_l1, weight=0.0)
